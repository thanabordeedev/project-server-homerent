...
<main-header navsel="back"></main-header>
  <div class="container">
    <h1>Edit Rent</h1>
    <form v-on:submit.prevent="editRent">
      ...
      <div class="form-group">
        <label for="InputBedroom" class="boldtext">Bedroom</label>
      ...
      <p>
        <button class="btn btn-warning" type="submit">update rent</button>
      </p>
    </form>
  </div>
  </div>
...
export default {
  ...
  methods: {
    async editRent() {
      ...
      try {
        await RentsService.put(this.rent);
        this.$router.push({
          name: "rents",
        });
      } catch (err) {
  ...
  async created() {
    ...
    try {
      ...
      this.pictures = JSON.parse(this.rent.pictures)
