<template>
  <div id="edit-comment">
    <h4 v-if="user && compUser.id === user.id" class="comment-user">
      {{ compUser.name }}
    </h4>
    ...
    <transition name="fade">
      <div v-if="updateEditable" class="form-edit-wrapper">
        <form v-on:submit.prevent="updateComment">
          ...
          <p><button type="submit" class="btn btn-warning btn-xs"><i class="fa fa-save"></i> Update</button>
            <button type="button" v-on:click.prevent="editComment(false, comment.id, compUser.id, user.id)" class="btn btn-success btn-xs"><i class="fa fa-times-circle"></i> Close</button>
          </p>
        </form>
      </div>
    </transition>
    <p v-if="!editable && user != null && compUser.id === user.id">
      <button v-on:click="deleteComment(comment)" class="btn btn-xs btn-danger"><i class="fa fa-trash-alt"></i> Delete</button>
    </p>
    <p v-else-if="user.type === 'admin'"><button v-on:click="deleteComment(comment)" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i> Delete</button></p>
  </div>
</template>
